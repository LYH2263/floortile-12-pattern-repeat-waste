...
function syncCycleFromTile() {
  const t = tiles.value.find(t => t.id === tileId.value)
  patternCycle.value = t?.pattern_cycle ?? 0
}

function cycleParam() {
  return Number(patternCycle.value) || 0
}

async function preview() {
  err.value = ''
  try {
    result.value = await getJSON(`/api/estimate?room_id=${roomId.value}&tile_id=${tileId.value}&pattern_cycle=${cycleParam()}`)
  } catch (e) {
    err.value = e.message
    result.value = null
  }
...
<template>
  <div class="page">
    ...
    <label>花铺循环长(m，0=关闭) <input type="number" min="0" step="0.1" v-model.number="patternCycle" /></label>
